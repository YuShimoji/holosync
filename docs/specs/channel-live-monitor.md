...
- [ ] チャンネルURL（/channel/UCxxx, /@handle）から監視登録できる
- [ ] ライブ配信中の動画が自動でタイルに追加される
- [ ] 複数チャンネル・複数同時ライブに対応
- [ ] 手動更新ボタンで即時チェック可能
- [ ] Page Visibility APIでタブ非表示時にポーリング停止
- [ ] 監視リストがlocalStorageに永続化される
- [ ] lint通過

## 未決事項

- [ ] ポーリング間隔をUI上で変更可能にするか（初期実装では固定15分でよいか）
