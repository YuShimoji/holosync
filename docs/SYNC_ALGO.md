...
### 3.8 Live Edge Sync (ライブ配信同期)

ライブ配信では通常のseekToベース同期が逆効果になる問題に対応。

**問題**: ライブ配信のcurrentTimeはDVRバッファ内の絶対秒数で返るため、
リーダーの再生位置にseekToするとフォロワーがライブエッジから遠ざかる可能性がある。

**判定**: `isLikelyLive(rec)` -- 以下のいずれかで判定
- `duration > 43200` (12時間超のDVRバッファ)
...
| P2 | ライブ配信間の同期精度向上 (エッジ追跡、バッファ量推定) | 精度 |
| P2 | least-bufferedモードのバッファリング比率計算 (現在は代替指標) | 精度 |
| P3 | デバッグパネルにdrift補正履歴表示 | 開発支援 |
| P3 | ライブ配信のグループ間遅延オフセット対応 | 機能 |
