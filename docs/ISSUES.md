...
- [x] feat(P2): レイアウトカスタマイズ（プリセット/編集/ドラッグ並べ替え）
  - 受け入れ基準:
    ...
    - タイルサイズ調整やドラッグ並び替え
    - レイアウト設定の保存・共有
  - 実装(2026-02-07): ツールバーにレイアウト選択UI（自動/1〜4列/シアター）。CSS Grid動的切替。レイアウト永続化。サイドバー折りたたみ。
  - **実装(Phase 5): フリーサイズリサイズ、セル配置モード、ドラッグ移動、行間余白制御を追加。**
...
- [x] feat(P2): キーボードショートカット（再生/停止/同期/音量/速度/選択）
  ...
  - 実装(2026-02-07): Space(再生/停止), M(ミュート), U(ミュート解除), F(フルスクリーン), S(同期), Shift+1〜9(タイル別フルスクリーン)

- [x] feat(P2): 音声コントロール拡張（個別ミュート/ソロ/正規化の下準備）
  - 受け入れ基準: 各タイルの個別音量調整とグローバル操作の両立
  - 実装(2026-02-07): オーディオフォーカス機能（タイルクリックで選択動画のみ音声再生、再クリックで全体復帰）
...
- [x] feat(P2): 動画情報表示UI（長さ/解像度/字幕/チャプター）
  ...
  - 実装(2026-02-07): 各タイルに開閉式情報パネル。oEmbed APIでタイトル/チャンネル取得（APIキー不要）。Data API利用時は概要も表示。
...
- [x] feat(P2): 埋め込みパラメータUI（controls/modestbranding/rel/playsinline）
  ...
  - 実装(2026-02-07): デフォルトでmodestbranding=1, rel=0を適用。
  - 実装(2026-02-14): サイドバーに埋め込み設定UIを追加。既存タイル再読込で反映し、embed設定を保存/共有URL復元に対応。
...
- [x] test(P2): E2Eテスト自動化（Playwright）
  - 受け入れ基準:
