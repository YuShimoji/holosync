...
- [ ] feat(P2): チャンネルLive監視（チャンネル登録→ライブ配信自動取得・再生）
  - 受け入れ基準: チャンネルURL入力→アクティブなライブ配信を検出→タイルに追加
  - 備考: search.list eventType=live（quota 100 units/call）。複数ライブ対応
