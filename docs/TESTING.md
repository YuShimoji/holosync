...
- [ ] 27. タイルのフリーサイズリサイズ
  - 操作: タイルにホバーし、右下の ⤡ ハンドルをドラッグ
  - 期待: タイルがピクセル単位で拡大/縮小される。リサイズ中にサイズバッジ（例: 480×270）が表示される。他のタイルのサイズ・位置は変わらない
- [ ] 28. フリー配置モードでのタイル移動
  - 操作: レイアウトを「フリー配置」に変更し、タイル上部の ⋮⋮ ハンドルをドラッグ
  - 期待: ドラッグ中にセルグリッドが半透明で表示され、ドロップ先がハイライトされる。ドロップ後にタイルがセル位置にスナップする
- [ ] 29. リサイズ・移動後の永続化
  - 操作: タイルをリサイズ/移動後、ページをリロード
  - 期待: リサイズしたサイズと配置位置が復元される
- [ ] 30. 間隔スライダーで余白調整
  - 操作: ツールバーの「間隔」スライダーを 0〜24px の範囲で変更
  - 期待: タイル間の余白が即座に変更される。フリー配置モードではセルグリッドも更新される
- [ ] 31. デバッグパネルの閉じるボタン
  - 操作: デバッグパネルを開き、×ボタンをクリック
  - 期待: パネルが正しく閉じる
