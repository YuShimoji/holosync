...
| 2026-04-14 | テスト過剰削減: E2E 15→10件 + INVARIANTS Test Discipline 追加          | 維持 / 削減 / 全廃                                               | 存在確認・重複テストは保守負担のみ。今後の件数増加を凍結し目的明確時のみ追加                                            |
| 2026-04-14 | SP-021/F-08 iframe sandbox 不要判定                                    | 付与 / 不要                                                      | cross-origin 境界で同等効果。setWindowOpenHandler全deny+明示外部起動APIで代替                                           |
| 2026-04-14 | SP-021/F-08 関連動画→新タイル追加を縮退                                | 実装 / 縮退                                                      | iframe cross-origin で URL捕捉不能。外部ブラウザ遷移防止の主目的は最小版で達成済み                                      |
| 2026-04-14 | SP-021/F-01 動画追加時デフォルト: 現状維持                             | 自動フィット / 現状維持                                          | 自動化と明示操作のバランスで明示優先                                                                                    |
| 2026-04-14 | SP-021/F-01 クイックフィットトリガー: Fキー + ボタン両方               | キーのみ / ボタンのみ / 両方                                     | 発見性(ボタン)と速度(キー)の両立                                                                                        |
| 2026-04-14 | SP-021/F-01 同時適用範囲: 全 chrome 非表示                             | 枠のみ / 枠+サイド / 全chrome                                    | 最大化視聴UXを優先。再トグル/ESCで即復元可能                                                                            |
| 2026-04-14 | SP-021/F-01 Electron自動ウィンドウサイズ: オプション既定OFF            | 実装しない / オプション / 既定ON                                 | 既存ユーザー挙動を壊さず将来拡張の余地を残す。16:9固定 (YouTube実動画ほぼ全て)                                          |
| 2026-04-15 | SP-021/F-09 shape select を radius slider に置換                       | 旧shape維持 / slider / プリセット併存                            | 柔軟性向上+UI簡素化。旧zoomShape値はresolveRadius()で自動変換                                                           |
| 2026-04-15 | SP-021/F-10 動画→ウィンドウは16:9仮定+グリッド列数で計算               | 実動画比取得 / 16:9仮定                                          | cross-originで実比取得不能。YouTube実動画ほぼ全てが16:9。列数からcols*16:rows*9で算出                                   |
| 2026-04-15 | .grid の drag region を撤廃、.content-toolbar を drag に変更           | scrollbar擬似要素でno-drag / .grid drag撤廃                      | Chromium は ::-webkit-scrollbar への app-region を信頼できずスクロール不可に。toolbar 空白領域を drag zone とし根本解決 |
