...
ブランチ戦略: main (トランクベース)
現フェーズ: SP-020 レイアウトUX摩擦の解消
直近の状態 (2026-03-19):

- SP-020 レイアウトUX摩擦5件を修正 (F-1〜F-5 全完了)
- P2バックログ5/6件完了 (残: YouTube OAuth履歴同期のみ)
- E2E 15/15 PASS, ESLint clean
- 仕様: 20エントリ (18 done + 1 deprecated + 1 superseded)
- SP-020: ツールバーsticky固定 / 動画追加後の自動レイアウト / 1列時高さ制限 / リサイズ追従 / スクロールバーvsドラッグ分離

## CURRENT DEVELOPMENT AXIS

- 主軸: レイアウトUX摩擦の解消 (SP-020)
- この軸を優先する理由: ユーザーフィードバックで5件の具体的UX摩擦を特定。動画追加→視聴の基本ワークフローに支障がある
- 今ここで避けるべき脱線: OAuth / Electron配布 / 新機能

## CURRENT LANE

- 主レーン: Experience Slice (UI/UX)
- 副レーン: なし
- 今このレーンを優先する理由: ユーザーが実際に遭遇している5件のUX摩擦を修正。基本ワークフローの品質向上
- いまは深入りしないレーン: Runtime Core, Acceptance/E2E拡充, ドキュメント整合

## CURRENT SLICE

- スライス名: SP-020 レイアウトUX摩擦の解消
- 操作列: 動画追加 → 自動レイアウト → リサイズ追従 → 1列最大化 → スクロール操作
- 成功条件: 動画追加後に手動でフィットモードを操作する必要がない / ツールバーが常時アクセス可能
- 基盤能力: fitmode.js レイアウト自動化 / CSS sticky/scrollbar-gutter
- 実装順: F-5→F-1→F-3→F-2→F-4
